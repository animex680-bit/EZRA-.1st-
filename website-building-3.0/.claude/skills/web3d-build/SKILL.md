...
## Tokens before generation (research 2026-06: Oliur/Relume, Gary Simon)

Define the design system FIRST — type scale, palette, spacing, component inventory — as a tokens file (`src/styles/tokens.css` or a theme object) derived from the signed-off concept. Every section/component is then generated against those tokens. This is the documented fix for "AI sites look generic": tokens-in, brand-out. Never generate sections before the tokens exist.

## Debug UI on day one (Bruno Simon doctrine)

Add `leva` (R3F) or `lil-gui` (vanilla) in the scaffold step, gated behind `?debug`. Live-tune camera, lights, uniforms, easings from the first scene — iteration speed is the real quality lever. Gate/strip it in production builds.

## WebGPU / TSL frontier (2026 shift — adopt deliberately)

Safari 26 made WebGPU universal and the creator meta (Mancini, Wawa Sensei, Greenheck, Heckel) is moving shaders from GLSL to **TSL** with `WebGPURenderer`. Policy: WebGL + GLSL remains our verified default; choose TSL/WebGPU when the signature moment needs it (GPGPU particles, SSGI-grade quality) AND the build can be render-verified — and keep the WebGL fallback path `WebGPURenderer` provides. Never hand-write r18x-era APIs from memory: work from current docs/reference implementations (base-model API knowledge is stale — Greenheck's webgpu-claude-skill exists for exactly this reason).
